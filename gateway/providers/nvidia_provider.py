...
class NvidiaProvider(BaseProvider):
    ...
    Uses OpenAI-compatible API format at https://integrate.api.nvidia.com
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        key_strategy = config.get("strategy", "fallback")
        ...
        logger.info(f"Initialized NVIDIA provider with base_url: {self.base_url}")
    ...
    def _get_headers(self, api_key: str) -> Dict[str, str]:
        ...
        return {
            ...
            "NVCF-STREAM": "true",  # Enable streaming support
        }

    def _map_model_name(self, model: str) -> str:
        """Map model alias to NVIDIA model name."""
        # Check if it's already a valid NVIDIA model ID
        if model.startswith("nvidia/") or "/" in model:
            return model
        ...
        default_mappings = {
            ...
            "smart": "nvidia/llama-3.1-nemotron-70b-instruct",
            "fast": "nvidia/llama-3.1-nemotron-70b-instruct",
        }

        if model in default_mappings:
            return default_mappings[model]

        # Return as-is, NVIDIA might handle it
        return model

    def _map_request(
        self, messages: List[Dict], model: str, stream: bool, **kwargs
    ) -> Dict:
        """Map gateway request to NVIDIA format."""
    ...
    async def chat_completion(
        ...
        """Send chat completion request to NVIDIA API with automatic key rotation on failure."""
        import httpx
        ...
        for attempt in range(self.max_retries * 2):
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(f"{self.name}: Key rate limited, retrying...")
                    continue

                if response.status_code in [402, 403]:
                    await self.key_manager.mark_quota_exhausted(key_obj)
                    logger.warning(f"{self.name}: Key quota exhausted, retrying...")
                    continue

                # Other errors
                await self.key_manager.mark_error(key_obj)
                logger.warning(f"{self.name}: API error {response.status_code}, retrying...")
                continue

            except (httpx.TimeoutException, httpx.NetworkError) as e:
                await self.key_manager.mark_error(key_obj)
                logger.warning(f"{self.name}: Request failure ({e}), retrying...")
                continue
    ...
    async def chat_completion_stream(
        ...
        """Stream chat completion from NVIDIA API with automatic key rotation on failure."""
        import httpx
    ...
    async def list_models(self) -> List[Dict[str, Any]]:
        """List available models from NVIDIA API."""
        import httpx
